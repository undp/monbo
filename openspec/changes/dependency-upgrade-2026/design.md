...
*Node 22 for the orchestrator vs. the runtime decision (D4):* the pinned `concurrently` (v10) declares `engines.node >=22`, so the root orchestrator effectively wants Node 22 from PR 1 — apparently earlier than D4/PR 8, which raises the *runtime* baseline to Node 22. There is no real contradiction: the orchestrator is a **dev-convenience tool, not a runtime**. The Node 22 runtime decision (`engines` in `monbo-front`, `node:22-alpine` Docker images, the CI runtime the app is built and shipped on) stays in PR 8 as D4 states; the root orchestrator merely adopts Node 22 for the dev shell that runs `pnpm dev`/`test`/`lint`/`build` from PR 1. Because pnpm's `engine-strict` is off, running the orchestrator on Node 20 emits a warning rather than failing, and each package can still be run directly on Node 20 from its own directory until PR 8. This is documented in the root README and `docs/onboarding.md`.
